...
function loadItem(json) {
  ...
  json.forEach((item, idx) => {
	...
	const options = item.PRODUCT_OPTION.split(',');
	let optionValue = "";
	options.forEach((option) => {
		if (option === item.USER_OPTION) {
			optionValue += `<option value=${option} selected>${option}</option>`;
		} else {
			optionValue += `<option value=${option}>${option}</option>`;
		}
	});
    ...
    `<tr>
	  ...
	  <td class="cart-option"><select name="pro_option_${idx}" class="option-select">`;
	html += optionValue;
    html += `</select></td>
  ...
  document.querySelectorAll('input[type="number"]').forEach((e) => {
	e.addEventListener('change', (event) => changeIten(event));
  });
...
function changeIten(event) {
	...
	let quantity = Number(trTag.querySelector('.cart-quantity input').value);
	let option = trTag.querySelector('.option-select').value;
	fetch(`cartUpdate.do?product_code=${pcode}&quantity=${quantity}&option=${option}`)
      .then(result => result.json());
